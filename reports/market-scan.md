# Morning Market Scanner

> **Research only — not investment advice. Verify all prices, catalysts, liquidity, and risk parameters with a live broker feed before trading.**

Generated: `2026-09-10T16:55:29.029802+00:00`  
Freshness: Data timestamp not supplied — verify freshness  
Schedule timezone: `America/Los_Angeles`  
Watchlist: **0 / 200 max**

## Applied filters

- **Price:** $1–$1000
- **Average Volume:** >1,000,000 shares (20-session mean)
- **Spread:** ≤0.3% of quote midpoint — NOT VERIFIED: provider published no data
- **Atr:** 2–8% (14 sessions)
- **Beta:** >0 vs SPY (60 sessions)
- **Rvol:** >1.5, same premarket time-window
- **Technical Structure:** clean MA trend + clear 20-session levels
- **Catalyst:** news, upcoming earnings, or ≥2% gap (required)
- **Watchlist Size:** up to 200; never padded
- **Volume Confirmation:** ≥1.1× prior 5-session mean volume
- **Volatility List:** separate daily list: ATR ≥5%, >1,000,000 avg shares, $1–$1000, spread ≤0.5%, RVOL ≥1; hard gates NOT applied
- **Momentum:** ≥15% over 63 sessions
- **Revenue Growth:** ≥10% latest quarter YoY (SEC XBRL)
- **Debt To Equity:** ≤3 (SEC XBRL)
- **Profitability:** ranking context only
- **Fundamentals Source:** SEC EDGAR filed company facts; an unfiled metric is left unevaluated

## Ranked watchlist

No stocks met every filter. No trade is a valid outcome.

## Warnings

- Yahoo Finance publishes no bid/ask quote; the tight-spread gate was NOT evaluated and remains unproven for every symbol listed.
- Yahoo Finance data is unofficial, may be delayed or adjusted without notice, and is rate-limited. Verify every price, level, and catalyst in a broker feed before trading.
- Yahoo request failed for 8 symbols: BITF, DFS, EXAS, LC, MRO, PLYA, SQ, X
- Yahoo returned complete price history for 236 of 245 requested symbols.
- SEC filings yielded no usable fundamentals for 6 symbols: SPOT, IHG, DLO, NU, STNE, PAGS
- Hard gates left unevaluated because the provider publishes no data for them: tight_spread. Those requirements are unproven for every symbol listed below.
- Only 0 symbols passed every hard gate; watchlist was not padded.
- The high-volatility list (9 names) is screened on liquidity and ATR ≥5% only. Those names have NOT passed the hard gates, carry no thesis, stop, or target, and are not a watchlist.
